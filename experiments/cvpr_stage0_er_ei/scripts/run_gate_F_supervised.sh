#!/usr/bin/env bash
# Step 3 / Gate F: supervised Fine-tune on stream D. One try.
# T1 knee Gaussian Cartesian 4× → T2 brain mini RSS + RandomMaskGenerator 8×.
# Main train loss = deepinv.loss.SupLoss (HQ MSE). MC/EI off.
# Do not start E (CT), three-arm, or claim-1 Avg rewrite.
set -euo pipefail
...
OUT="${1:-${ROOT}/recorded_smoke/stage0b_gate_F_supervised}"
T1="${2:-80}"
T2="${3:-160}"
mkdir -p "${OUT}"
START_EPOCH=$(date +%s)
...
python3 -m cvpr_stage0_er_ei --unit finetune --no-tiny --seed 1 --gate F --supervised \
...
ELAPSED=$((END_EPOCH - START_EPOCH))
{
  ...
  echo "train_loss=deepinv.loss.SupLoss (HQ MSE; MC/EI off)"
  echo "stream=T1 knee Gaussian Cartesian 4x -> T2 brain mini RSS RandomMaskGenerator 8x"
  echo "claim_scope=Step-3 supervised Fine-tune on stream D (domain+operator composite)"
  echo "device=${DEVICE}"
  echo "arm=finetune"
  echo "n_train=6"
  echo "n_eval=2"
