#!/usr/bin/env python3
"""Aggregate light-UNet N_buf=1 seeds {1,2,3} README + metrics JSON/CSV.

Separate from MoDL recorded_smoke/stage1_crossip_grid/. Formal S1 go/kill
at N_buf=1 only: ≥2/3 seeds with Fgt(ER+EI) < Fgt(ER+MC).
"""
...
def _load_cell(out_dir: Path, seed: int) -> dict | None:
    ...
    if seed == 1:
        alt = ROOT / "recorded_smoke" / "light_unet_s1_seed1" / "metrics_nbuf1_seed1.json"
        if alt.is_file():
...
def main(argv: list[str]) -> int:
    ...
    payload = {
        "label": "light-UNet N_buf=1 seeds 1-3 (formal S1 at N_buf=1 only)",
        "not_modl": True,
        "separate_from": "recorded_smoke/stage1_crossip_grid/",
        "backbone": backbone,
        "supervised": False,
        "unsupervised": True,
        "cross_ip": True,
    ...
    write_json(out_dir / "metrics_light_unet_s1.json", payload)
    if rows:
        write_csv(out_dir / "metrics_light_unet_s1.csv", rows)

    n_params = backbone.get("n_parameters")
    lines = [
        "# Light-UNet N_buf=1 seeds 1–3 (formal S1 at N_buf=1)",
        "",
        "Stream E three-arm cells on **`deepinv.models.UNet(in_channels=2, "
        "out_channels=2, scales=3)`** wrapped by `ArtifactRemoval(mode=adjoint)`. "
    ...
    lines.extend(
        [
            ...
            "",
            "Rule: ≥2/3 seeds with Fgt(ER+EI) < Fgt(ER+MC). With a single N_buf "
            "point, majority-of-N is that one point. Not an N_buf∈{1,2,4,8} matrix.",
            "",
            f"- verdict: **{go.get('verdict')}**",
            f"- reason: {go.get('reason')}",
            f"- majority seeds: `{go.get('majority_seeds')}` "
            f"({go.get('n_majority_seeds')}/3)",
            f"- seeds present: `{sorted(cells)}` missing=`{missing}`",
            f"- device: `{payload.get('device')}`",
            "",
            "## Reviewer probes",
            "",
            "Per-cell JSON must keep: `physics_class_t2` Tomography(40), "
            "`n_distinct_ya==1`, `replay_rebuilds_mri` via `make_mri_physics`, "
            "nonzero `BufferReplayLoss` on ER arms, backbone UNet+ArtifactRemoval.",
            "",
        ]
    )
    ...
    print(f"Wrote {out_dir / 'metrics_light_unet_s1.json'}")
