...
OPTIONAL_WEIGHTS_DIR = Path("weights")
...
def get_model(model_name, device='cpu', channels=1):
    ...
    if model_name == 'dncnn':
        model = dinv.models.DnCNN(in_channels=channels, out_channels=channels, device=device, pretrained='download')
    elif model_name == 'alpha_dncnn':
        dncnn = dinv.models.DnCNN(in_channels=channels, out_channels=channels, device=device, pretrained='download')
        model = alpha_DnCNN(dncnn)
    elif model_name == 'lip_dncnn':
        model = dinv.models.DnCNN(in_channels=channels, out_channels=channels, device=device, pretrained='download_lipschitz')
    elif model_name == 'drunet':
        model = dinv.models.DRUNet(in_channels=channels, out_channels=channels, device=device, pretrained=str(OPTIONAL_WEIGHTS_DIR / ('drunet_' + col_str + '.pth')))
    elif model_name == 'gs_drunet':
        model = dinv.models.GSDRUNet(in_channels=channels, out_channels=channels, device=device, pretrained=str(OPTIONAL_WEIGHTS_DIR / 'GSDRUNet.ckpt'))
    elif model_name == 'drunet_deepinv':
        model = dinv.models.DRUNet(in_channels=channels, out_channels=channels, device=device, pretrained=str(OPTIONAL_WEIGHTS_DIR / ('drunet_deepinv_' + col_str + '.pth')))
    elif model_name == 'diffunet':
        model = dinv.models.DiffUNet(in_channels=channels, out_channels=channels, pretrained=str(OPTIONAL_WEIGHTS_DIR / 'diffusion_ffhq_10m.pt'))
    elif model_name == 'diffunet_large':
        model = dinv.models.DiffUNet(in_channels=channels, large_model=True, out_channels=channels, pretrained=str(OPTIONAL_WEIGHTS_DIR / 'diffusion_openai.pt'))
    elif model_name == 'scunet':
        model = dinv.models.SCUNet(in_nc=channels, device=device,
                                   pretrained=str(OPTIONAL_WEIGHTS_DIR / ('scunet_' + col_str + '_real_psnr.pth')))
    elif model_name == 'swinir':
        col_str = '005_color' if channels == 3 else '004_gray'
        model = dinv.models.SwinIR(in_chans=channels, device=device,
                                   pretrained=str(OPTIONAL_WEIGHTS_DIR / (col_str + 'DN_DFWB_s128w8_SwinIR-M_noise15.pth')))
